      ...
      return (
        ...
        divideFunct(
          ...
          Number(entry.totalTokensBidded) /
            Math.pow(10, Number(pureDecimals || 18))
        )
      );
    }, 0)
  : 0;
...
export const pureOverallUSDBidded = pureLeaderboard
  ? pureLeaderboard.reduce((total, entry) => {
      return (
        total +
        Number(entry.totalUSDBidded) / Math.pow(10, Number(pureDecimals || 18))
      );
    }, 0)
  : 0;
...
export const pureBurntTokens = getTokenBurnt
  ? (
      Number(getTokenBurnt) / Math.pow(10, Number(pureDecimals || 18))
    ).toLocaleString()
  : 0;
...
export const pureAbyssUSDPrice_s = pureAbyssUSDPrice
  ? (
      Number(pureAbyssUSDPrice) / Math.pow(10, Number(pureDecimals || 18))
    ).toLocaleString()
  : 0;

export const pureBurntTokensUSDValue =
  pureBurntTokens || pureAbyssUSDPrice
    ? (
        Number(pureBurntTokens) *
        (Number(pureAbyssUSDPrice) / Math.pow(10, Number(pureDecimals || 18)))
      ).toLocaleString()
    : 0;

export const pureBidAmountUSDValue =
  pureAbyssUSDPrice || pureBidAmount
    ? (
        (Number(pureBidAmount) / Math.pow(10, 18)) *
        (Number(pureAbyssUSDPrice) / Math.pow(10, 18))
      ).toLocaleString()
    : 0;
